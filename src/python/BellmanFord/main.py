...
def bellmanFord(edges, source, n):
    ...
    for k in range(n - 1):
        # edges from `u` to `v` having weight `w`
        for (u, v, w) in edges:
            # if the distance to destination `v` can be shortend by taking edge (u, v)
    ...
    for (u,v, w) in edges:  # edge from `u` to `v` having weight `w`
        # if the distance to destination `u` can be shortend by taking edge (u, v)
        if distance[u] != sys.maxsize and distance[u] + w < distance[v]:
...
if __name__ == '__main__':
    ...
    # of graph edfes as per the above diagram 
    edges = [
